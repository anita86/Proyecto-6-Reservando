...
Listado.prototype.buscarRestaurante = function(id) {
  var restauranteBuscado = this.restaurantes.find(restaurant => restaurant.id === id);
...
Listado.prototype.obtenerUbicaciones = function() {
  var ubicaciones = this.restaurantes.map(restaurant => restaurant.ubicacion);
  var ciudadesFiltradas = eliminarRepetidos(ubicaciones);
...
Listado.prototype.obtenerRubros = function() {
  ...
  var rubrosFiltrados = eliminarRepetidos(rubros);
...
Listado.prototype.obtenerHorarios = function() {
  ...
  var arreglosDeHorarios = this.restaurantes.map(restaurant => restaurant.horarios);

  //En este arreglo vamos a poner todos los horarios, uno por uno
  var horariosTotales = arreglosDeHorarios.reduce(function(a, b) {
    return a.concat(b);
  });

  var horariosFiltrados = eliminarRepetidos(horariosTotales);

  return horariosFiltrados.sort();
}

//Función que recibe los filtros que llegan desde el HTML y filtra el arreglo de restaurantes. Solo se filtra si el valor recibido es distinto de null.
